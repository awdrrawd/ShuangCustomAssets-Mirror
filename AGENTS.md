...
[2026-07-25] 新增不可信域名提示功能 - 配置页面添加开关(开/关)和透明度调节，开启时不可信域名贴图替换为警告图(固定参数167,-256,16%,旋转0,透明度可调)，关闭时跳过不可信域名不加载不渲染 - src/assets/customTexture.js, src/assets/settings.js
