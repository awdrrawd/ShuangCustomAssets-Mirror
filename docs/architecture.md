...
SCA does not register AssetManager.enableFromModUserValidation: that helper writes the shared appearance-diff parameter `fromModUser`, which other asset plugins also use. SCA checks removal/swap sources only when the previous asset is 自定义贴图 and the target is the local player. Self changes, source 0, and sources with SCA_INFO continue through normal game/mod validation. Other assets and all shared parameters pass through unchanged. The inventory visibility validator remains enabled. Live multiplayer verification with Echo is still required.
